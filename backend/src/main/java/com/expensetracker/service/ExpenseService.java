...
public class ExpenseService {
    ...
    public ExpenseDTO addExpense(Long groupId, ExpenseDTO expenseDTO, String currentUserEmail) {
        ...
        if (expenseDTO.getSplits() == null || expenseDTO.getSplits().isEmpty()) {
            ...
            for(SplitDTO splitDTO : expenseDTO.getSplits()) {
                ...
                BigDecimal splitAmountINR = splitDTO.getAmount().multiply(rate).setScale(2, RoundingMode.HALF_UP);
    ...
    private void distributeEqually(Expense expense, ExpenseGroup group, List<ExpenseSplit> splits) {
        ...
        BigDecimal totalAmount = expense.getAmount(); // This is already INR
        BigDecimal splitAmount = totalAmount.divide(BigDecimal.valueOf(memberCount), 2, RoundingMode.DOWN);
        BigDecimal remainder = totalAmount.subtract(splitAmount.multiply(BigDecimal.valueOf(memberCount)));
